...
import { getFilePostMetadata } from '$lib/content';
...
export async function load({ params, url }) {
	...
	// Check file-based content first
	const fileMetadata = getFilePostMetadata(slug);
	if (fileMetadata) {
		return { source: 'file', metadata: fileMetadata, slug };
	}

	// Fall back to Firestore
	try {
		const postDoc = await adminDB.collection('posts').doc(slug).get();
		...
		return { source: 'firestore', post, categoryHierarchy };
	} catch (err) {
